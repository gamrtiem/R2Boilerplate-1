...
{
    public class BoilingThermos : MonoBehaviour 
    {
        ...
        private void RecalculateStatsAPI_GetStatCoefficients(RoR2.CharacterBody sender, RecalculateStatsAPI.StatHookEventArgs args)
        {
            if(sender)
            {
                ...
                if(count != 0)
                {
                    Log.Info("starting moveSpeedMultAdd: " + args.moveSpeedMultAdd);
                    ...
                    Log.Info("Ending moveSpeedMultAdd: " + args.moveSpeedMultAdd);
                }
            }
        }
